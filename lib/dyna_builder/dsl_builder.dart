...
class DslBuilder extends GeneratorForAnnotation<DynaBlock> {
  @override
  generateForAnnotatedElement(
    ...
    print('[dynaFlutter] Start Compile patch');
    print('[dynaFlutter] Element： $element');
    print('[dynaFlutter] Element name ： ${element.name}');
    print('[dynaFlutter] Element display name ： ${element.displayName}');

    print('[dynaFlutter] Element source： ${element.source}');
  ...
  dynamic _buildWidgetDsl(Expression? widgetExpression) {
    ...
    if (methodInvocationExpression?.callee?.isIdentifier == true) {
      print('MCLOG==== isIdentifier name : ${methodInvocationExpression?.callee?.asIdentifier.name}');
      ...
      if (RegExp(r'^[a-z_]')
        ...
        dslMap.putIfAbsent(
            'name', () => methodInvocationExpression?.callee?.asIdentifier.name);
      }
...
Builder dynaDsl(BuilderOptions options) => LibraryBuilder(DslBuilder());
